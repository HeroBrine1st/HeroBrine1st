...
[^1]: Forward soundness (also called forward logic safety) is a technique I (independently) came up with to prevent *future* bugs in codebases by disallowing making inconsistent changes by delegating consistency checks to compiler. It requires language support for restricting data flow paths, e.g. compiler can disallow converting an instance of `value class UserId(private val id: Int)` to `value class PostId(private val id: Int`, so unsafe usage can be restricted. This means not only ensuring soundness but preventing (most of) future errors and contract violatons - that's why it's forward.  
DRY is also included: copying code allows to desync the code, so you should have logic without duplicates. It works like 2NF (where 1NF is SRP) - make links (foreign keys) to existing code (columns in foreign tables) instead of copying them.  
Excluding some natural suspicion to race conditions, that's how I mostly avoid debugging.
